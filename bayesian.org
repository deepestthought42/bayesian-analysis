...
(defparameter *d*
  (tof-data:data *test-file* 1
		 :min-tof 20d0 :max-tof 50d0 :min-no-ions 1 :max-no-ions 1))
#+END_SRC
...
(labels ((cmd (fmt-str &rest args)
  ...
  (mgl-gnuplot:with-session ()
    ...
    (cmd "set terminal x11 enhanced font 'Georgia,8' dashed")
;    (cmd "set logscale x")
    (let ((d (ba::laplacian-approximation *res* 'fp::rho-m0 300 :collect-log nil)))
     (mgl-gnuplot:plot*
      (list
;       (mgl-gnuplot:data* d "u 1:2 with steps title 'f - d'")
;       (mgl-gnuplot:data* d "u 1:3 with steps title 'f'")
       (mgl-gnuplot:data* d "u 1:4 with steps title 'd'")
       )))
    (cmd "unset output")))


(ba::laplacian-approximation *res* 'fp::tof-offset 100 :collect-log t)
