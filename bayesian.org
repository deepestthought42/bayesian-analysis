...
;     (make-instance 'ba:metropolis-hastings :no-iterations 50000)
...
(labels ((cmd (fmt-str &rest args)
  ...
  (mgl-gnuplot:with-session ()
    ...
    (mgl-gnuplot:plot*
     (list
      (mgl-gnuplot:data*
       (map 'list #'identity (ba::laplacian-approximation *res* 'fp::rho-m0 100 :on-center nil))
       "with steps"))) 
    (cmd "unset output")))
...
- mmmh, laplacian-approximation doesn't work anymore for rho-m0 ?
...
- laplacian-approximation sets slots, but it makes copies, let's test
  ...
  (labels ((cmd (fmt-str &rest args)
    ...
    (mgl-gnuplot:with-session ()
      (cmd "reset")
      (cmd "set terminal wxt enhanced font 'Georgia,8' dashed")
      (ba::laplacian-approximation *res* 'fp::rho-m0 100 :on-center nil)
      (ba::laplacian-approximation *res* 'fp::om-c 100 :on-center nil)
      (mgl-gnuplot:plot*
       (list
        (mgl-gnuplot:data*
         (map 'list #'identity (ba::laplacian-approximation *res*
                                                            'fp::tof-offset 300
                                                            :on-center nil))
         "with steps"))) 
      (cmd "unset output")))
