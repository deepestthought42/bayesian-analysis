...
(ba:define-bayesian-model (line toy-data)
    ((temperature :default 2d0 :prior :jeffreys :marginalize t :min 5d-2 :max 100d0 :sample-sigma 0.5d0)
     ...
     (sigma-l :default #.(/ 2 (log 2))))
    (:d_i=f_i+gaussian_error_1_equal_sigma
     :equal-sigma-parameter (sigma :default 1d0 :marginalize nil))
    ((nu)
      ($ temperature * (exp (- (nu - nu-0) ^ 2 / (2 * sigma-l ^ 2))))))
#+END_SRC
...
(let+ ((p-res (ba:get-parameter-results *mcmc-result* :start 2000 :no-bins 200))
  ...
  (plot (:file-name "example/nu0.png") ("set yrange [0:1.1]")
    ((normalize-to-one
      (ba::parameter-pdf-integrate '(nu-0 1d0 64d0) 200 '((temperature 1d-1 5d0))
				   (make-instance 'line)
				   (ba:initialize-from-source 'toy-data t)
				   :normalize t))
