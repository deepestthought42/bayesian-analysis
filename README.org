...
Metropolis-Hastings algorithm (with 200000 iterations in this instance).
#+BEGIN_SRC lisp :results none :package :bayesian-example
...
(let+ ((p-res (ba:get-parameter-results *mcmc-result* :start 2000 :no-bins 200))
  ...
  (plot (:file-name "example/nu0.png")
    ...
    ((normalize-to-one ba:binned-data) "with histeps lc 0 title 'MCMC result (200000 iterations)'")))
#+END_SRC
